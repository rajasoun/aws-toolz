...
def main():
    ...
    try:
        ...
        disable_account_alias = os.environ.get(
            'AWS_WHOAMI_DISABLE_ACCOUNT_ALIAS', '')
...
def format_whoami(whoami_info):
    ...
    if whoami_info.SSOPermissionSet:
        ...
        type_str = ''.join(p[0].upper() + p[1:]
                           for p in whoami_info.Type.split('-'))
...
def whoami(session=None, disable_account_alias=False):
    ...
    if data['Type'] == 'assumed-role' and data['Name'].startswith('AWSReservedSSO'):
        try:
            ...
            data['SSOPermissionSet'] = data['Name'].split('_', 1)[
                1].rsplit('_', 1)[0]
        except Exception as e:
    ...
    if not disable_account_alias:
        try:
            # pedantry
            paginator = session.create_client(
                'iam').get_paginator('list_account_aliases')
